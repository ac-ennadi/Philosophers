...
/*   Updated: 2025/08/13 12:21:52 by acennadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*died_check(void *args)
{
	...
	while (philo->config->stop != 1)
	{
		...
		if (((philo->config->now
					- philo[philo->config->i].last_meal)
				>= philo->config->time_to_die))
		{
			stdout_lock(philo->config, &philo[philo->config->i], "is died");
		...
		if (philo->config->number_of_times_each_philosopher_must_eat != -1)
			is_all(philo);
		philo->config->i++;
		if (philo->config->i >= philo->config->number_of_philosophers)
			philo->config->i = 0;
		usleep(25);
	}
	return (NULL);
